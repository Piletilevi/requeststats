...
//console.log($bgColors)
 function popChartTotalDurations() {
...
function popChartTotalRequests() {
	 ...
	 Object.keys($names).map(function(key, i) {
		 $tooltips[i] = [];
		 $labels[i] = key;
		 $requests[i] = $names[key].Requests;
		 $requestsSuccess[i] = $names[key].Success;
		 $requestsFail[i] = $names[key].Requests - $names[key].Success;
		 //formatter_dec_0.format(dataString);
		 $tooltips[i]['total'] = 'Total Durations:  ' + formatter_dec_0.format($names[key].Durations);
		 $tooltips[i]['success'] = 'Success Durations:  ' + formatter_dec_0.format($names[key].DurationsSuccess);
		 $tooltips[i]['fail'] = 'Failed Durations:  ' + formatter_dec_0.format((parseInt($names[key].Durations) - parseInt($names[key].DurationsSuccess)));
		 count = i + 1;
	 });
     var requestMax = Math.max.apply(null, $requests);
     var $backgroundColorsRGBA=[];
                 ...
                 if (!meta.hidden) {
					 meta.data.forEach(function(element, index) {
                      ...
                      //   ctx.font = 'italic 30px sans-serif';
                         ...
                         ctx.strokeStyle = textColor['total'];
                         if (dataStringTotal[index] !== 'is'){
                             dataStringTotal[index] = formatter_dec_0.format($requests[index]);
                             ctx.fillText(dataStringTotal[index]+' =', 260+i*80, position.y +  (fontSize*lineHeight-fontSize)/2);
                             dataStringTotal[index] = 'is';
                         }
					 });
...
*/
				 {
					 category: 'success',
					 label: "total of Success Requests",
				 ...
				 {
					 category: 'fail',
					 label: "total of Failed Requests",
					 data: $requestsFail,
					 backgroundColor: $backgroundColorsRGBA['fail'],
					 borderWidth: [0, 0, 0, 0],
				 },
